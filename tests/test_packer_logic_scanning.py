...
def test_start_order_packing_already_completed(loaded_logic):
    ...
    loaded_logic.process_sku_scan(
        "SKU-CCC"
    )  # completes the only item -> ORDER_COMPLETE
...
def test_scan_completes_order_only_when_every_item_done(loaded_logic):
    ...
    loaded_logic.process_sku_scan(
        "SKU-AAA"
    )  # SKU-AAA fully packed, SKU-BBB still pending
    _result, status = loaded_logic.process_sku_scan("SKU-BBB")
    assert status == "ORDER_COMPLETE"
    assert (
        loaded_logic.current_order_number == "ORDER-001"
    )  # not auto-cleared; caller's job
...
def test_scan_beyond_required_quantity_is_extra_not_silently_accepted(loaded_logic):
    ...
    _result, status = loaded_logic.process_sku_scan(
        "SKU-CCC"
    )  # second scan of a qty-1 item
...
def test_sku_map_translates_barcode_to_internal_sku(
    packer_logic_factory, session_factory, profile_manager
):
    orders = [
        (
            "ORDER-001",
            "DHL",
            [{"sku": "INTERNAL-SKU-1", "quantity": 1, "product_name": "Widget"}],
        )
    ]
    ...
    result, status = logic.process_sku_scan(
        "7290018664100"
    )  # scan the manufacturer barcode
    assert (
        status == "ORDER_COMPLETE"
    )  # single item, qty 1 -> this scan finishes the order
...
def test_cancel_after_order_complete_is_rejected_because_current_order_is_cleared(
    loaded_logic,
):
...
def test_force_confirm_completes_item_and_records_remaining_qty(loaded_logic):
    ...
    assert (
        loaded_logic.current_order_items_scanned[-1]["confirmation_method"]
        == "force_confirmed"
    )
...
def test_force_confirm_on_partially_scanned_item_only_adds_remaining_record(
    loaded_logic,
):
    loaded_logic.start_order_packing("ORDER-001")
    ...
    force_records = [
        r
        for r in loaded_logic.current_order_items_scanned
        if r["confirmation_method"] == "force_confirmed"
    ]
    assert len(force_records) == 1
    assert (
        force_records[0]["quantity"] == 1
    )  # only the remaining unit, not the full requirement
...
def test_extra_scan_on_a_not_yet_complete_order_does_not_finish_it(loaded_logic):
    ...
    loaded_logic.process_sku_scan(
        "SKU-AAA"
    )  # SKU-AAA done (2/2), SKU-BBB still pending
...
def test_order_complete_with_extras_status_when_last_required_item_scanned(
    packer_logic_factory, session_factory
):
    ...
    _result, status = logic.process_sku_scan(
        "SKU-2"
    )  # completes required qty for every item
...
def test_confirm_keep_extra_completes_order_once_all_extras_resolved(loaded_logic):
    ...
    loaded_logic.process_sku_scan(
        "SKU-AAA"
    )  # SKU_EXTRA, current_extra_items = {"skuaaa": 1}
    loaded_logic.process_sku_scan(
        "SKU-BBB"
    )  # completes required qty everywhere -> WITH_EXTRAS
